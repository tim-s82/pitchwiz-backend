...
- **`users/views.py:68`** — **`ChangePasswordView` comment leaks intent**: `permission_classes = [] # or IsAuthenticated if you handle auth globally/locally` is a TODO-style comment left in production code. **Recommendation:** use `permission_classes=[IsAuthenticated]`, remove the manual check, and delete the comment.

- **`bookings/views.py:471`** — **`requests` module not imported**: `sync_play_cricket_fixtures_view` calls `requests.get(...)` but `requests` is never imported at the top of `bookings/views.py`. This is a `NameError` at runtime. **Fix:** add `import requests` to the imports at the top of the file.
...
- **`src/App.jsx:477`** — **Typo in class name**: `text-slate-405` does not exist in Tailwind's default palette (`405` is not a valid shade). This silently produces no colour, likely defaulting to transparent. The intended value was probably `text-slate-400`. **Fix:** change to `text-slate-400`.
